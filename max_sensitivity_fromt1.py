...
def optimumrecycledelaytwoexp(x, ampl1, tau1, tau2):
    ...
    x_vals = np.linspace(0.1, x, 100000)
    y_vals = lam_x(x_vals)
    y_vals_bup = lam_x2(x_vals)
    return x_vals, y_vals_bup, x_vals[np.argmin(abs(y_vals))]
...
if option == 'Monoexponential':
    ...
    x, y, optd1 = optimumrecycledelayoneexp(max_time, tb)
...
if option == 'Biexponential':
    ...
    max_time = st.number_input('Enter the maximum time in the buildup: ', value = 512.0)
    x, y, optd1 = optimumrecycledelaytwoexp(max_time, amp1, ta, tb)
